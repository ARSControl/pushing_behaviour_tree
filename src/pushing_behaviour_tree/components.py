...
class CheckObjectInTarget(py_trees.behaviour.Behaviour):
        ...
        def update(self):
                ...
                norm = sqrt((obj.position.x - target.position.x)**2 + (obj.position.y - target.position.y)**2)
                if  norm < 0.01:
...
class DetachfromObj(py_trees.behaviour.Behaviour):
        ...
        def initialise(self):
                ...
                self.blackboard.psafe= pr + V*np.array([math.cos(thetar+math.pi),math.sin(thetar+math.pi)])
...
class ComputeTrajectory(py_trees.behaviour.Behaviour):
        ...
        def setup(self):
                self.feedback_message="setup"
                self.get_plan = rospy.ServiceProxy('/pushing_planner/GetPushingPlan',GetPushingPaths)
                self.plan = []
